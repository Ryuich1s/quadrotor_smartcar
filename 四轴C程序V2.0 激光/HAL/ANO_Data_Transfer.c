...
//			cnt = 0;	f.send_pid = 0;	
	...
	{		
		...
		switch(cnt)
		{
			case 1:	ANO_DT_Send_Status(-Angle.roll,Angle.pitch,-Angle.yaw,mini.flow_High,ALL_flag.slock_flag,ALL_flag.unlock);	 // 飞控状态    10001
			break;
			case 2:	
							ANO_DT_Send_Senser(MPU6050.accX,MPU6050.accY,MPU6050.accZ,					//	陀螺仪加速度数据
//																 MPU6050.gyroX,MPU6050.gyroY,MPU6050.gyroZ, 			//	陀螺仪角度数据
//																 mini.flow_x_iOUT,mini.flow_y_iOUT , pixel_flow.Att_Err_ACC);  //	光流原使数据	

																Flow_PosPid_y.desired,Flow_PosPid_x.desired,pidHeightHigh.desired, 			//	陀螺仪角度数据
																 pixel_flow.loc_y,pixel_flow.loc_x , mini.flow_High);  //	光流原使数据			
			break;
			...
			break;
			case 4:	ANO_DT_Send_Power(voltage/10,Remote.AUX5,1,NRF_SSI,test_flag,set_flag);   //飞机电压/遥控器电压/无线模块的信号值/传感器状态/飞行模式  
			break;
			case 5:	 ANO_DT_Send_Senser2(FlightData.High.bara_height,Control_high);  //气压计/实时控制的高度
			break;
